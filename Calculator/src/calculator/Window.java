...
public class Window extends javax.swing.JFrame {
    ...
    public Window()
    {
        ...
        // points to current line in expressions
        pointer = 0;
        ...
        // don't have to worry about focus
        ...
        else
        {
            ...
            switch (command)
            {
                ...
                case "=":
                    ...
                    try
                    {
                        start = expressions.getLineStartOffset(pointer);
                        int end = expressions.getLineEndOffset(pointer);
                        int length = end - start;

                        int expressionCount = expressions.getLineCount() - 1;

                        if (pointer == expressionCount)
                        {
                            /**
                        ...
                        String expression = expressions.getText(start, length);
                        ...
                        // else calculate current expression
        ...
        switch (key)
        {
            case KeyEvent.VK_UP:
                if (pointer > 0)
                {
                    ...
                    {
                        start = expressions.getLineStartOffset(pointer);
                        end = expressions.getLineEndOffset(pointer);
            ...
            case KeyEvent.VK_DOWN:
                // don't include newest expression
                int expressionCount = expressions.getLineCount() - 1;

                // zero-based lines
                if (pointer < expressionCount - 1)
                {
                    pointer++;
    ...
    // highlight colors
    private Color error = new Color(255, 0, 0, 100); // light red
    private Color selection = new Color(0, 0, 255, 100); // light blue
